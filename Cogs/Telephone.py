...
class Telephone:
	...
	async def telenumber(self, ctx):
		...
		description = ("Hey **{}**! The phonenumber for this server is given below.".format(author.name))
		field_name = ":telephone: PhoneNumber"
		field_contents = teleNumFormat
		footer_text = ""

		embed = discord.Embed(colour=0xFF0000, description=description)  # Can use discord.Colour()
		embed.title = ""
		embed.set_thumbnail(url='http://www.clker.com/cliparts/0/f/c/2/1195445181899094722molumen_phone_icon.svg.hi.png ')
	...
	async def _ring(self, caller, receiver, hidden, dial_hide):
		...
		author = ctx.message.author
		description = (":telephone_receiver: **{}** You have been connected to the call\n\nType **hangup** to end the call")
		field_name = ""
		field_contents = ""
		footer_text = ""
		...
		await receiver_chan.send(embed=embed)
		# Wait on the call
		while True:
			# Setup the check
